...
export const sellerService = {
  ...
  getFollowedSellers: async (userId) => {
    try {
      const response = await fetchWithTimeout(`${BASE_URL}/api/buyer/following`, {
        method: 'GET',
        headers: buildHeaders(userId),
      });
      const text = await response.text();
      let data;
      try { data = JSON.parse(text); } catch (e) { throw new Error(`Invalid JSON: ${text.slice(0, 100)}`); }
      if (!response.ok) throw new Error(data.error || `Failed to fetch followed sellers (${response.status})`);
      return data;
    } catch (error) {
      console.error('getFollowedSellers error:', error);
      throw error;
    }
  },
};
